...
export default function FindAccountView() {
  ...
  const handleAction = async () => {
    ...
    try {
      ...
      alert(tab === 'id' 
        ? "입력하신 이메일로 아이디 정보를 보냈습니다! ✉️" 
        : "임시 비밀번호가 발송되었습니다. 메일함을 확인해주세요! 🔑"
      );
    } catch (error) {
      console.error("계정 찾기 오류:", error);
      const errorMessage = error.response?.data?.message || "등록된 정보가 없습니다. 다시 확인해주세요. 🧐";
      alert(errorMessage);
    } finally {
      setLoading(false);
    }
      ...
      <main className="flex-1 px-8 pt-8 pb-12">
        ...
        <div className="space-y-6">
          <div className="group relative">
            <Mail className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-blue-600 transition-all" size={22} />
            <input
              type="email"
